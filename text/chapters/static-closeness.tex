...
\begin{algorithm2e}[h!]
 ...
 \KwData{$G = (V, E), v, x_k$}
 \KwResult{A tuple $(h, isExact)$ with $isExact = $\texttt{false} if $h$ is only an upper bound for the exact harmonic closeness}
 Create queue $Q$ \\
 $Q$.enqueue($v$) \\
 Mark $v$ as visisted \\
 ...
 \While{!Q.isEmpty}{
   ...
   \ForAll{$w \in N(u)$}{
     \uIf{$w$ is not marked as visited}{
       ...
       $\widetilde{\gamma} \gets \widetilde{\gamma} + outdegree(w) - 1 \cdot G.isDirected()$ \label{alg:gammaUpdate}
     }
...
\paragraph{Computing level-based bounds}
The level bound is 
\begin{align}
...
% TODO: The harmonic case can't be simplified to use only prefix and suffix sums
...
\begin{algorithm2e}[h!]
 ...
 \While{$Q$ is not empty}{
   $v \gets \texttt{Q.extractMax()}$ \\
   
   \If{$|\texttt{Top}| \geq k \land $\texttt{score[$v$]}$ \leq $ \texttt{Top[$k$]}}{
     return \texttt{Top}
   } 
   
   \texttt{score[$v$]} $\gets $ \texttt{updateBounds($v$)} \\
   add $v$ to \texttt{Top} \\
   sort \texttt{Top} by \texttt{score} and reduce it to at most $k$ elements \\
   update \texttt{Q} according to the new values in \texttt{score}
...
\begin{algorithm2e}[h!]
 ...
 \KwResult{The exact closeness centrality of $s$, upper bounds for the closeness of all other nodes}
  ...
  \For{$i = 0$ \KwTo \texttt{maxD}}{
    $\Gamma_i \gets \{w \in V : d(s, w) = i\}$ \\
    $\gamma_i \gets |\Gamma_i|$ \\
    \texttt{sum}$\Gamma_{\leq i} \gets $\texttt{sum}$\Gamma_{\leq i - 1} + \gamma_i$ \\
     \texttt{sum}$\Gamma_{> i} \gets |V| - \texttt{sum}\Gamma_{\leq i}$ \\
  }
